...
def test_update_user(session: Session, client: TestClient, superuser_token_headers: dict[str, str]):
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=session, user_create=user_in)
    data = {"full_name": "Updated_full_name"}
    r = client.patch(f"{settings.API_V1_STR}/users/{user.id}",
                     headers=superuser_token_headers, json=data)
    assert r.status_code == 200
    updated_user = r.json()
...
def test_update_user_not_exists(client: TestClient, superuser_token_headers: dict[str, str]):
    data = {"full_name": "Updated_full_name"}
    r = client.patch(f"{settings.API_V1_STR}/users/{uuid.uuid4()}",
                     headers=superuser_token_headers, json=data)
    assert r.status_code == 404
    assert r.json()[
        "detail"] == "The user with this id dose not exist in the system"


def test_update_user_email_exists(session: Session, client: TestClient, superuser_token_headers: dict[str, str]):
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=session, user_create=user_in)
...
def test_delete_user_me(session: Session, client: TestClient):
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=session, user_create=user_in)
...
def test_delete_user_me_as_superuser(client: TestClient, superuser_token_headers: dict[str, str]):
...
def test_delete_user_super_user(session: Session, client: TestClient, superuser_token_headers: dict[str, str]):
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=session, user_create=user_in)
    user_id = user.id
    r = client.delete(f"{settings.API_V1_STR}/users/{user_id}",
                      headers=superuser_token_headers)
    assert r.status_code == 200
    delete_user = r.json()
    assert delete_user["message"] == "User deleted successfully"
    user_query = select(User).where(User.id == user_id)
    user_db = session.exec(user_query).first()
    assert user_db is None


def test_delete_user_not_found(client: TestClient, superuser_token_headers: dict[str, str]):
    r = client.delete(
...
def test_delete_user_current_super_user_error(session: Session, client: TestClient, superuser_token_headers: dict[str, str]):
...
def test_delete_user_without_privileges(session: Session, client: TestClient, normal_user_token_headers: dict[str, str]):
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=session, user_create=user_in)

    r = client.delete(f"{settings.API_V1_STR}/users/{user.id}",
                      headers=normal_user_token_headers)
    assert r.status_code == 403
    assert r.json()["detail"] == "The user doesn't have enough privileges"
